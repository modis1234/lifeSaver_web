define([
	...
	"jsc",
	"w2ui",
	"text!views/gasMonitor_v2",
	"text!views/severityPanel",
	"text!views/cctvPanel",
	"css!cs/stylesheets/ls_v2/main.css",
	"css!cs/stylesheets/ls_v2/layout.css",
	"css!cs/stylesheets/ls_v2/interaction.css",
	...
	return Backbone.View.extend({
		...
		getCctvList: function (model, response) {
			...
			_this.cctvInfo['fan_address'] = _result['fan_address'];
			_this.cctvInfo['port'] = _result['port'];
			_this.cctvInfo['user_id'] = _result['user_id'];
			_this.cctvInfo['password'] = _result['password'];
        },
		...
		doSocketConn: function () {
			...
			_this.socket.emit('sendIndex', window.main.sensorIndex);
		...
		getData: function () {
			...
			_socket.emit('joinRoom', window.main.sensorIndex);
		...
		alarmPlayHandler: function (event) {
			...
			// _audio.src = 'http://119.207.78.144:9099/upload/' + fileName;
			_audio.volume = 1;
			_audio.loop = true;
			_audio.play();
		...
		cctvRender: function () {
			var _this = this;
			var objectLeng = _this.$el.find('.camBox').find('.view').find('object').length;
			if (objectLeng === 0) {
				this.$el.find('.camBox').find('.view').append(cctvPanel);
				_this.cctv = new Idis('WatSearCtrl');
				_this.cctv.init(_this.cctvInfo);
			}
		},
		onFullScreen: function () {
			var _this = this;
			if (_this.cctv) {
				_this.cctv.onFullScreen();
			}
		},
		onRender: function () {
			var _this = this;
			...
			_this.$el.find('.count').find('.description').css('display', 'block');
			_this.$el.find('.count').find('.ready').text('공기질 측정 중입니다.');
			_this.$el.find('.count').find('.description').text('경과 되었습니다.');
		...
		offRender: function () {
			...
			_this.$el.find('.count').find('.time').css('display', 'none');
			_this.$el.find('.count').find('.time').css('display', 'none');
			_this.$el.find('.count').find('.ready').text('시스템 준비중입니다.');
			_this.$el.find('.count').find('.description').css('display', 'none');
		...
		loadingRender: function () {
			...
			_this.$el.find('.count').find('.description').css('display', 'block');
			_this.$el.find('.count').find('.description').text('후 검지를 시작합니다.');
		...
		chartShow: function (event) {
			...
			if (!hasClaz) {
				...
				var _gasType = _this.$el.find(_target).attr('value');
		...
		rollingPlayEvent: function(){
			...
			_this.$el.find('span#rolling-stop-btn').css('display','block');
			_this.rollingAction = true;
			_this.rollingInterval = setInterval(function () {
				_this.buttonRolling();
			}, _this.rollingCount);
		...
		rollingPauseEvent: function(){
			...
			_this.$el.find('span#rolling-start-btn').css('display','block');
		...
		leaveRoomListener: function(event){
			console.log(event.target)
			var _this = this;
			_this.socket.emit('leaveRoom', window.main.sensorIndex)
		},
		events: {
			"click .chart-list": "chartShow",
			"mousedown .chart-list": "rollingTargetChange",
			"click .expand": "onFullScreen",
			...
			"mousedown .adm": "leaveRoomListener"
		},
		destroy: function () {
			var _this = this;
			_this.gasValue['vocValue'] = 0;
